...
export default {
  ...
  methods: {
    ...
    async init() {
      ...
      try {
        let { resultCode, data } = await this.$api.ProductSetting.queryProductList(this.currentPage, this.pageSize);
        console.log(data);
        if (+resultCode === 200) {
    ...
    async handleRemove(id) {
      ...
      try {
        let { resultCode } = await this.$api.ProductSetting.removeProduct(ids);
        if (+resultCode === 200) {
    ...
    async handleUpdate(row) {
      ...
      try {
        let { resultCode, data } = await this.$api.ProductSetting.info(row.configId);
        if (+resultCode === 200) {
          this.ruleFrom = data;
    ...
    async Submit() {
      try {
        ...
        //添加
        let fun = this.$api.ProductSetting.addProduct,
          data = { ...this.ruleFrom };

        //修改
        if (data.goodsId) {
          fun = this.$api.ProductSetting.updataProduct;
